...
>(
  (
    ...
    const [open, setOpen] = React.useState<boolean | null>(null); // Initialize to null

    // https://chatgpt.com/c/675102f2-f584-8007-b907-11b380d41af7
    // note: as of this commit i have turned away from default handling to allow for better SSR/client rendering support
    // Read state from localStorage after component mounts
    React.useEffect(() => {
      if (typeof window !== "undefined") {
        const storedValue = localStorage.getItem(SIDEBAR_COOKIE_NAME);
        setOpen(storedValue ? storedValue === "true" : defaultOpen);
      } else {
        setOpen(defaultOpen);
      }
    }, [defaultOpen]);
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      {...props}
    >
      <SidebarIcon className="hidden size-6 text-muted-foreground md:block" />
      <Menu className="size-6 text-muted-foreground md:hidden" />
